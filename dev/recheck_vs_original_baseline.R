...
# Warm-up sequence matching the earlier regression harness order.
...
run_case <- function(label, expr, iterations) {
  ...
  invisible(eval(expr_sub, envir = expr_env))
  gc(full = TRUE)
  bm <- bench::mark(
...
now_tbl <- do.call(rbind, list(
  run_case("polychoric_ci_p", polychoric(lat_ord, ci = TRUE, p_value = TRUE), 2L),
  run_case("pearson_pairwise_ci", pearson_corr(X_na[, 1:20], na_method = "pairwise", ci = TRUE), 2L),
  run_case("ccc_rm_reml_ci", ccc_rm_reml(ccc, response = "y", subject = "subject", method = "method", time = "time", ci = TRUE, ar = "none"), 2L),
  run_case("ccc_rm_ustat_ci", ccc_rm_ustat(ccc, response = "y", subject = "subject", method = "method", time = "time", ci = TRUE), 2L),
  run_case("dcor_pairwise_inf", dcor(X_na[, 1:20], na_method = "pairwise", p_value = TRUE), 2L),
  run_case("summary_dcor_top", summary(dcor(X[, 1:20]), topn = 25L), 3L),
  run_case("dcor_matrix", dcor(X[, 1:20]), 3L)
))
