...
This vignette focuses on classical and robust CCC for wide data and uses
...
When discordant outliers can dominate the ordinary moments, `robust_ccc()`
replaces the means, variances, and covariance in Lin's coefficient with joint,
reweighted minimum covariance determinant (MCD) estimates. The estimator is
intended for data with an approximately elliptically symmetric, unimodal core;
...
The classical and robust estimators can be compared directly because both use
the standard matrixCorr result interface:

```{r}
contaminated <- data.frame(m1 = m1, m2 = m2)
contaminated[1, ] <- c(145, 65)

fit_ccc_contaminated <- ccc(contaminated)
fit_robust_ccc <- robust_ccc(contaminated, seed = 40)

c(
  classical = estimate(fit_ccc_contaminated)[1, 2],
  robust = estimate(fit_robust_ccc)[1, 2]
)
```

In `robust_ccc()`, lower `alpha` values retain a smaller MCD subset and provide
greater resistance to contamination. The default `alpha = 0.75` balances
robustness and efficiency. Set `seed` when reproducibility matters because the
FastMCD search is stochastic. Optional confidence intervals use a paired
percentile bootstrap and can be computationally expensive because the MCD fit
is repeated for every pair and resample.
...
`robust_ccc()` is the robust wide-data counterpart for contamination-prone
measurements. It retains Lin's coefficient form but substitutes pairwise joint
reweighted MCD location and scatter estimates. It supports optional paired
percentile-bootstrap confidence intervals, but does not implement a formal
hypothesis test. Sparse and edge-list outputs use the same absolute-threshold
semantics as other matrixCorr estimators.
...
agreement_summary <- data.frame(
  ...
  `Target question` = c(
    ...
    "Pairwise concordance resistant to contamination in an elliptical, unimodal core",
  ...
  `Estimation approach` = c(
    ...
    "Lin CCC form with joint reweighted MCD location and scatter; paired percentile-bootstrap CI when requested",
  ...
  `Inference reported` = c(
    ...
    "Estimate; optional paired percentile-bootstrap confidence interval",
...
- Use `robust_ccc()` when wide-data concordance is the target, discordant
  outliers are a concern, and an approximately elliptical, unimodal core is a
...
Hubert M, Debruyne M, Rousseeuw PJ. (2018). Minimum covariance determinant and
extensions. *WIREs Computational Statistics*, 10, e1421.
