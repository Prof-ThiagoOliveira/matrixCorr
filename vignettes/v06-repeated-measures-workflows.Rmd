...
The repeated-measures API is mostly, but not perfectly, harmonised. `rmcorr()`,
`ba_rm()`, and `ccc_rm_ustat()` use a `subject` argument, while the REML-based
`ccc_rm_reml()` and `icc_rm_reml()` use `rind` for the same identifier role.
