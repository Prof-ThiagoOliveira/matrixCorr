...
```{r}
set.seed(21)
X <- as.data.frame(matrix(rnorm(200 * 6), ncol = 6))
...
sample correlation can become unstable. `schafer_corr()` provides a shrinkage
...
n <- 40
p <- 120
Xd <- matrix(rnorm(n * p), nrow = n, ncol = p)
colnames(Xd) <- paste0("G", seq_len(p))

fit_shr <- schafer_corr(Xd)
...
where the sample matrix is noisy or singular.
...
- If the concern is `p >= n` or unstable covariance estimation, use
  `schafer_corr()` or a regularised `pcorr()` method.
